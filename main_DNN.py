...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 18:57:29 2024
...
parser.add_argument('--input-dataset', default='dataset/16LiFi/10UE_3Nf_withR/input_MixedGraph_5000.csv')
parser.add_argument('--output-dataset', default='dataset/16LiFi/10UE_3Nf_withR/output_MixedGraph_5000.csv')
...
parser.add_argument('--model-name', default='DNN', help='basic network-centric DNN for resource allocation in MPTCP LiFi')
############   Defaulte parameters   ############
parser.add_argument('--exp_name', default='Description: DNN model predicts RA in MPTCP-enabled HLWNets')
parser.add_argument('--epoch-num', default=51, type=int)
parser.add_argument('--learning-rate', default=1e-04, type=float, help='Learning rate')
...
args.folder_to_save_files = 'results/Final/16LiFi/'+datetime.datetime.now().strftime("%Y-%m-%d-%I-%M-%S-%p-")+'%s'%args.model_name+'-%sUE'%args.UE_num+'-%sNf'%args.Nf+'-withR'
...
for epoch in range(args.epoch_num):
    ...
    for batch_data_input, batch_data_label in train_loader:
        ...
        pred_label = model(batch_data_input)
        
        pred_final_label = pred_label.view(args.batch_size, args.UE_num, args.Nf)
        
        loss = criterion(pred_label, resized_labels)
    ...
    if epoch % args.test_freq == 0:
        ...
        with torch.no_grad():
            for batch_data_input, batch_data_label in test_loader:
                ...
                ######   calcualte MAE   ######
                abs_diff = torch.abs(pred_label - resized_labels)
                mae += abs_diff.mean()
        ...
        print('Ground-truth throughput (Mbps) is %s, predicted DNN throughput is %s, and Gap is %s'%(aver_gt_thr, aver_pred_thr, aver_gap))
        print('Ground-truth Jains fairness is %s, predicted DNN fairness is %s;'%(aver_gt_fairness, aver_pred_fairness))
